...
function install_aws_deps {
  ...
  if [[ "$OSTYPE" == linux-gnu* ]]; then
    wget https://dl.min.io/server/minio/release/linux-$MINIO_OS/archive/minio.RELEASE.2022-05-26T05-48-41Z -O minio
  elif [[ "$OSTYPE" == darwin* ]]; then
    # minio will have to approved under the Privacy & Security on MacOS on first use.
    wget https://dl.min.io/server/minio/release/darwin-$MINIO_OS/archive/minio.RELEASE.2022-05-26T05-48-41Z -O minio
  fi
  chmod +x ./minio
