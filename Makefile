...
veloxminiapp:
	$(MAKE) cmake BUILD_DIR=${BUILD_DIR} BUILD_TYPE=${BUILD_TYPE} EXTRA_CMAKE_FLAGS="${EXTRA_CMAKE_FLAGS} -DVELOX_ENABLE_PARQUET=ON -DVELOX_BUILD_TESTING=OFF -DVELOX_BUILD_TEST_UTILS=ON -DVELOX_ENABLE_EXAMPLES=ON"
	cmake --build $(BUILD_BASE_DIR)/$(BUILD_DIR) --target=velox_mini_app -j $(NUM_THREADS)
