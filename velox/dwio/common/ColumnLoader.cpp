...
RowSet read(
  ...
  // 'loaded' close to 'offered' means the LazyVector deferred work that
  // happened anyway.
  LOG(INFO) << "Lazy load: column=" << fieldReader->scanSpec()->fieldName()
            << " offered=" << outputRows.size()
            << " loaded=" << effectiveRows.size()
